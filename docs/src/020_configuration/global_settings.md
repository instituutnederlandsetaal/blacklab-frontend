...
description: Configuring the server setup [corpus-frontend.properties]
order: 0
---

# Global Settings

Corpus-Frontend is configured through a file **`corpus-frontend.properties`** _(as long as you don't deploy the Frontend under a different path/name, see below)_.  
...
4. The directory where the corpus-frontend `.war` file is located 
...
The **file name and relative location** of the `corpus-frontend.properties` is actually taken from the **`context path` of the application**.  
...
Adding extra segments to the context path (e.g. `/corpora/frontend/`, instead of `/corpus-frontend/`) will also add those segments to the file path.
...
|-----------------------|-------------------------|--------------------------------------------- |-------------------------|-----------------------------------------|
| `corpus-frontend.war` | —                       | —                                            | `/corpus-frontend`      | `corpus-frontend.properties`            |
| `my-frontend.war`     | —                       | —                                            | `/my-frontend`          | `my-frontend.properties`                |
| any                   | `/test/corpus-frontend` | —                                            | `/test/corpus-frontend` | `test/corpus-frontend.properties`       |
| any                   | `/text/corpus-frontend` | `/frisian-corpora` → `/TEST/corpus-frontend` | `/frisian-corpora`      | `test/corpus-frontend.properties`       |
...
You can leave out any setting, and the Corpus Frontend will just use the default value for that setting.  
