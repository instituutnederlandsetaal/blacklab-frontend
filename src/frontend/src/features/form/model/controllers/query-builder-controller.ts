...
import type { CqlAnnotationCombinator, CqlAttributeData, CqlAttributeGroupData, CqlGroupEntry, CqlQueryBuilderData, CqlQueryBuilderOptions } from '@/widgets/cql-query-builder/model';
import { createDefaultCqlQueryBuilderData, isCqlAttributeData, isCqlAttributeGroupData } from '@/widgets/cql-query-builder/model';
...
function tokenToPattern(builderToken: QueryBuilderFieldState['tokens'][number]): CqlPattern | null {
	...
	const repeated = hasRepeat(builderToken.properties)
		? repeat(body, builderToken.properties.minRepeats, builderToken.properties.maxRepeats, builderToken.properties.optional)
		: body;
