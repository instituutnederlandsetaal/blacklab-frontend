...
import { CancelableRequest } from './loadable-streams';
 ...
 * @returns CancelableRequest resolving to cached or fetched data
 */
export function cachedRequest<T>(key: string, options: CachedRequestOptions): CancelableRequest<T> {
	...
	const requestPromise = axios.get<T>(options.baseURL + options.url, { ...config, cancelToken: source.token })
		.then(response => {
	...
	return new CancelableRequest(requestPromise, source.cancel);
}
