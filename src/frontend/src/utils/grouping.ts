...
function determineRelationPartField(results: BLSearchResult | undefined, label: string, relationPart: string | undefined, overriddenFieldName: string | undefined) {
	...
	if (matchInfoDef) {
		...
		else return matchInfoDef.fieldName ?? defaultFieldName;
	}
...
function parseCriterium(criterium: string, results?: BLSearchResult): SortingOrGroupingCriterium {
	...
	switch (type) {
		...
		case 'context': {
			...
			if (parsedSpec) {
				// this can contain more, like ; and a second(+) set of positions. We'll ignore that for now.
...
export function isValidGroupBy(g: GroupBy): boolean {
	...
	if (g.type === 'metadata') {
		if (g.metadata.type === 'document') return !!g.metadata.field;
		if (g.metadata.type === 'span-attribute') return !!(g.metadata.spanName || g.metadata.attributeName);
	}
	if (g.type === 'context') {
	...
	function baseHumanize(g: GroupBy | SortBy): string {
		if (g.type === 'context') {
			...
			switch (g.context.position) {
				case 'A':
					positionDisplay = i18n.$t('results.groupBy.summary.position.after').toString();
					break;
				case 'B':
					positionDisplay = i18n.$t('results.groupBy.summary.position.before').toString();
					break;
				case 'H':
					positionDisplay = i18n.$t('results.groupBy.summary.position.hit').toString();
					break;
				case 'E':
					positionDisplay = i18n.$t('results.groupBy.summary.position.end').toString();
					break;
				default:
					positionDisplay = g.context.position;
			}

			switch (g.context.whichTokens) {
				case 'all':
					return i18n.$t('results.groupBy.summary.allWords', { field: field, position: positionDisplay }).toString();
				case 'first':
					return i18n.$t('results.groupBy.summary.firstWord', { field: field, position: positionDisplay }).toString();
				case 'specific':
					return i18n
						.$t('results.groupBy.summary.indexedWord', {
							field: field,
			...
			if (meta.type === 'span-attribute') {
				...
				return filter ? i18n.$tMetaDisplayName(filter).toString() : i18n.$tSpanAttributeDisplay(span, meta.attributeName).toString();
			}
			// Document-level metadata
			return meta.field
...
export function humanizeSerializedGroupBy<T extends string | string[]>(i18n: Translate, g: T, annotations: Record<string, NormalizedAnnotation>, metadata: Record<string, NormalizedMetadataField>): T {
