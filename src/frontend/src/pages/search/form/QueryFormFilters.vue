	...
	<div>
		...
		<template v-if="useTabs">
			...
			<div class="tab-content">
				<div v-for="(tab, i) in tabs" :class="['tab-pane', 'form-horizontal', 'filter-container', { active: activeTab === tab.tabname }]" :key="tab.tabname" :id="tab.tabname">
		...
		<div v-else-if="allFilters.length" class="tab-content form-horizontal filter-container">
			<!-- TODO don't use tab-content when no tabs -->
			<Component
...
export default defineComponent({
	...
	methods: {
		updateFilterValue(id: string, value: any) {
			FilterStore.actions.filterValue({ id, value });
		},
		...
		synchronizeActiveTab() {
			if (this.activeTab == null || !this.tabs.find(t => t.tabname === this.activeTab)) this.activeTab = this.tabs[0]?.tabname ?? null;
		},
	},
	computed: {
		activeTab: {
			get(): string | null {
				return InterfaceStore.getState().activeFilterTab;
			},
			set(value: string | null) {
				InterfaceStore.actions.activeFilterTab(value);
			},
		},
		...
		allFilters(): FilterStore.FullFilterState[] {
			...
			return this.tabs
				.flatMap(t => t.subtabs.flatMap(tab => tab.fields.map(id => filterMap[id])))
				.filter(f => {
		...
		useTabs(): boolean {
			return this.tabs.length > 1 || (this.tabs.length > 0 && this.tabs[0].subtabs.length > 1);
		},
		activeFiltersMap(): Record<string, number> {
			...
			const manuallyActiveFiltersInCurrentTab: Record<string, boolean> = activeTab
				? mapReduce(
						activeTab.subtabs.flatMap(subtab =>
			...
			this.tabs.forEach(tab => {
				if (tab === activeTab) {
					...
					numActiveFiltersPerTab[tab.tabname] = tab.subtabs.reduce(
						(num, subtab) =>
							num +
							subtab.fields.filter(
								filter =>
			...
			handler(cur: string, prev: string) {
				...
				if (curQuery) {
					...
					Object.entries(curQuery).forEach(([id, value]) => {
						...
						const actualValue = valueFuncs.decodeInitialState(id, filter.metadata, { [id]: { id: id, values: value } }, undefined as any);
						...
						this.cancelFilterWatch.push(
							watch(
								() => FilterStore.getState().filters[id].value,
								(cur, prev) => {
									if (cur != prev) {
										FilterStore.actions.filterValue({
											id,
