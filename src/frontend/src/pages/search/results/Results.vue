	...
	<div>
		<ul id="resultTabs" class="nav nav-tabs cf-panel-tab-header cf-panel-lg">
			<li v-for="v in customViews" :class="[{ active: viewedResults === v.id }]" :title="v.title">
				<a href="javascript:void(0);" @click="showView(v.id)">{{ v.label || v.title || v.id }}</a>
			</li>
		</ul>

		<div class="tab-content cf-panel-tab-body cf-panel-lg" style="padding-top: 0px">
			<component v-for="v in customViews" :is="v.component" :key="v.id" v-show="viewedResults === v.id" :id="v.id" :active="viewedResults === v.id" :store="storeFor(v.id)"></component>
		</div>
	</div>
</template>
