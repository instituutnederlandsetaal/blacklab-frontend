	...
	<div class="cf-panel cf-panel-lg">
					...
					<tr>
						...
						<td class="corpus-name">
							<router-link
								:to="{ name: 'search', params: { corpus: corpus.id } }"
								:title="`Search the '${corpus.displayName}' corpus`"
								:class="`${!corpus.canSearch ? 'disabled' : ''}`"
						...
						<template v-if="isPrivate">
							...
							<td>
								<a
									role="button"
									:title="`Share the '${corpus.displayName}' corpus`"
									class="icon fa fa-fw fa-user-plus"
									@click="$emit('share', corpus.id)"
								></a>
							</td>
							<td>
								<a
									role="button"
									:title="`Delete the '${corpus.displayName}' corpus`"
									:class="`icon fa fa-fw fa-trash ${!corpus.canIndex ? 'disabled' : ''}`"
									@click="$emit('delete', corpus.id)"
								></a>
							</td>
						</template>
						<td>
						...
						<td :colspan="(isPrivate ? 7 : 4) + (debug ? 1 : 0)">
							<table><tbody>
								...
								<tr v-if="isPrivate">
									...
									<td
										:title="
											corpus.format && corpus.format.owner
												? 'Format owned by ' + corpus.format.owner
												: ''
										"
									>
										{{ corpus.format && corpus.format.owner ? "*" : ""
										}}{{ corpus.format ? corpus.format.shortId : corpus.documentFormat }}
									</td>
								</tr>
								<tr>
									<th>Description</th>
		...
		<div v-if="isPrivate">
			...
			<div v-else class="text-danger" style="padding-left: 8px">
				<em
					>You have reached the private corpora limit.<br />You will have to delete one of your
					corpora before you may create another.</em
				>
			</div>
		</div>
	</div>
...
export default defineComponent({
	...
	computed: {
		debug(): boolean { return debug.value; },
