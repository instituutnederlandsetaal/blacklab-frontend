<template>
	<div>
		...
		<button
			...
			title="select subannotations of the main annotation"
			:disabled="defaultSubAnnotations.length === 0"
			@click="$emit('update:modelValue', { ...modelValue, subAnnotations: defaultSubAnnotations })"
		>
			select defaults
		</button>
		<div style="display: flex; gap: 15px; padding: 10px 0; align-items: flex-start; width: 100%">
			<div class="unpicked" style="display: inline-flex; flex-direction: column; gap: 2px">
				<h3>Selected</h3>
				<button
					v-for="a in modelValue.subAnnotations"
					:key="a.id"
					type="button"
					class="btn btn-default"
			...
			<div class="unpicked" style="display: inline-flex; flex-direction: column; gap: 2px">
				<h3>Available</h3>
				<button
					v-for="a in unpickedSubAnnotations"
					:key="a.id"
					type="button"
					class="btn btn-default"
...
const step = defineComponent({
	...
	computed: {
		defaultSubAnnotations(): NormalizedAnnotation[] {
			return this.modelValue.annotations.filter(a => a.parentAnnotationId === this.modelValue.mainPosAnnotationId);
		},
