	...
	<div>
		...
		<div>
			<ul v-for="(values, annotId) in displays" :key="annotId" class="list-unstyled annotation-list" style="padding: 0 5px">
				<li class="annotation-id">
					<strong>{{ annotId }}</strong>
				</li>
				<li v-for="(display, value) in values" :key="value" class="annotation-value">
					<label :for="`${annotId}-${value}`">{{ value }}</label>

					<input type="text" v-model="displays[annotId][value]" :placeholder="value" style="width: 200px" :id="`${annotId}-${value}`" :style="{ borderColor: display === value ? 'red' : undefined }" />
				</li>
			</ul>
		</div>
...
function getDisplayNamesFromTagset(t: Tagset, mainAnnotationId: string, annotationCaseSensitivities: Record<string, boolean> = {}): Record<string, Record<string, string>> {
	...
	Object.values(t.values).forEach(({ value, displayName }) => {
		...
		if (!annotationCaseSensitivities[mainAnnotationId]) value = value.toLowerCase();
	...
	Object.values(t.subAnnotations).forEach(sub => {
		...
		sub.values.forEach(({ value, displayName }) => {
			const originalValue = value;
			if (!annotationCaseSensitivities[sub.id]) value = value.toLowerCase();
...
export const step = defineComponent({
	...
	computed: {
		...
		decodedDisplayNames(): Record<string, Record<string, string>> | { default: Record<string, string> } {
			if (this.importValueAsTagset)
				return getDisplayNamesFromTagset(this.importValueAsTagset, this.modelValue.mainPosAnnotationId!, Object.fromEntries(this.modelValue.annotations.map(a => [a.id, a.caseSensitive])));
