import { isLoaded, Loadable, type LoadableLike } from '@/_new/utils/loadable/loadable';
...
export type PassthroughFrom<T extends MaybeLoadablesInput> =
	T extends MaybeLoadable<unknown>[] ? T[number] : T[keyof T];
...
export function firstNonLoaded<T extends MaybeLoadablesInput>(loadables: T): PassthroughFrom<T> | undefined {
	if (Array.isArray(loadables)) 
		return loadables.find(v => !isLoaded(v)) as PassthroughFrom<T> | undefined;
