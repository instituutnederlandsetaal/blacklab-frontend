// TODO this should import the customization entries for isolated features/pages, and group them together
// then expose them on window, or as some part of page-customization hook
// the net effect of this will be that the pages/features are decoupled from customization of other pages/features
// we can just introduce a config/customization.ts file for each page/feature and import all of those here (or in the app somewhere).

import { isObject } from '@vueuse/core';
import { nextTick } from 'vue';

import type { CorpusChange } from '@/_new/app/plugins/installCorpusData';
import type { FilterDefinition, NormalizedAnnotation, NormalizedIndex } from '@/_new/types/apptypes';
import type { BLHitResults, BLHitGroupResults, BLDoc, BLHit, BLHitInOtherField, BLHitSnippet } from '@/_new/types/blacklabtypes';

import { spanFilterId } from '@/_new/shared/blacklab-helpers/span-filters-helper';
import type { HighlightSection } from '@/_new/shared/hit-highlighting';
import type { Translate } from '@/_new/shared/i18n/i18n';
import type { OptGroup, Option } from '@/_new/shared/utils/options';
...
export const corpusCustomizations = wrapWithErrorHandling({
	...
	/** index information */
	_corpus: dontProxy({}) as NormalizedIndex | null,
	...
	search: {
		pattern: {
			 ...
			 */
			uiType(fieldName: string, annotationName: string): NormalizedAnnotation['uiType'] | null {
				return null;
			},
		...
		metadata: {
			...
			createSpanFilter(
				...
				displayName: string,
				metadata: unknown = {},
			): FilterDefinition<{ name: string; attribute: string; options?: Option[] }> {
	...
	results: {
		 ...
		 */
		hasCustomHitInfoColumn: (results: BLHitResults | BLHitGroupResults, isParallelCorpus: boolean): boolean => {
		 ...
		 */
		customHitInfo: (hit: BLHit | BLHitSnippet | BLHitInOtherField, annotatedFieldDisplayName: string | null, docInfo: BLDoc): string | null => {
