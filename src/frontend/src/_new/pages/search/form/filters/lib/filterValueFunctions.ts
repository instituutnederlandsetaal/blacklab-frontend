...
import type { FullFilterState } from '@/_new/pages/search/form/filters/store/filter-store';
import type { FilterValue } from '@/_new/types/apptypes';

import { spanFilterId } from '@/_new/shared/blacklab-helpers/span-filters-helper';
import { debugLog } from '@/_new/shared/debug/debug';
import { mapReduce } from '@/_new/shared/utils/map-reduce';
import { optionLabel, findOption, optionValues, type Option } from '@/_new/shared/utils/options';
import { escapeLucene, splitIntoTerms, unescapeLucene } from '@/_new/shared/utils/string-utils';
...
function getFieldValues(
	...
	while (stack.length) {
		...
		// terminating clause
		if (isRange(cur.left) && (cur.left.field === field1 || cur.left.field === field2) && isRange(cur.right) && (cur.right.field === field1 || cur.right.field === field2)) {
			...
			return {
				...
				mode: cur.operator === '&&' ? 'strict' : cur.operator === 'AND' ? 'strict' : cur.operator === '||' ? 'permissive' : cur.operator === 'OR' ? 'permissive' : 'permissive', // unknown operator -- just interpret as permissive.
			};
		}
...
export const valueFunctions = {
	...
	'filter-range-multiple-fields': {
		decodeInitialState(id, filterMetadata, filterValues, ast) {
			...
			// https://github.com/instituutnederlandsetaal/blacklab-frontend/issues/379
...
} satisfies Record<string, FilterValueFunctions<any, any>>;
...
export function getValueFunctions(filter: { behaviourName?: string; componentName?: string }): FilterValueFunctions<any, any> {
	...
	const func = valueFunctions[name as any as keyof typeof valueFunctions];
	if (func) return func;
...
export function getFilterString(filters: FullFilterState[]): string | undefined {
	return (
		filters
			...
			.filter(lucene => !!lucene)
			.join(' AND ') || undefined
	);
}

// NOTE: range filter has hidden defaults for unset field (min, max), see https://github.com/instituutnederlandsetaal/blacklab-frontend/issues/234
export const getFilterSummary = (filters: FullFilterState[]): string | undefined =>
	filters
		.map(f => {
