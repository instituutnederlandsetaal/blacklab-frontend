...
import { escapeRegex } from '@/_new/shared/utils/string-utils';
...
const get = {
	ngram: {
		...
		groupBy: useMemoize(() => `hit:${state.ngram.groupAnnotationId}`),
		patternString: useMemoize(() =>
			state.ngram.tokens
	...
	frequency: {
		...
		patternString: () => '[]',
		groupBy: useMemoize(() => `hit:${state.frequency.annotationId}`),
	},
