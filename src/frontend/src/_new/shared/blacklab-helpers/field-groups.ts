...
import type { NormalizedAnnotation, NormalizedAnnotationGroup, NormalizedMetadataGroup } from '@/_new/types/apptypes';
...
export function getAnnotationSubset(
	ids: string[],
	groups: NormalizedAnnotationGroup[],
	...
	if (operation === 'Search') {
		...
		].map<OptGroup & { entries: NormalizedAnnotation[] }>(([prefix, groupname, suffix]) => ({
			label: groupname,
