import type { ParsePatternResponse } from "@/_new/shared/api";
import type { Endpoint } from "@/_new/shared/api/lib/api-endpoint";
import type { BlackLabApi } from "@/_new/shared/api/lib/api-types";
import { combineRequests, rejectedRequest } from "@/_new/shared/api/lib/api-utils";
import { ApiError } from "@/types/apptypes";
import type { BLAnnotatedField, BLDocGroupResults, BLDocResults, BLDocument, BLFormatContent, BLFormats, BLHit, BLHitGroupResults, BLHitResults, BLHitSnippetPart, BLIndex, BLRelationInfo, BLResponse, BLSearchParameters, BLServer, BLShareInfo, BLTermOccurances } from "@/types/blacklabtypes";
...
export const blacklabPaths = {
	...
	*/
	...
	autocompleteAnnotation: (
		...
		annotationId: string) =>                    `${endpoint.defaults.baseURL}${indexId}/autocomplete/${annotatedFieldId}/${annotationId}/`,
	// Is used outside the axios endpoint we created above, so prefix with the correct location
	autocompleteMetadata: (
		indexId: string,
		metadataFieldId: string) =>                 `${endpoint.defaults.baseURL}${indexId}/autocomplete/${metadataFieldId}/`,
 ...
 * Blacklab api
 */
export const createBlackLabApi = (endpoint: Endpoint): BlackLabApi => ({
	...
	getParsePattern: (indexId: string, pattern: string, requestParameters?: AxiosRequestConfig) => {
		if (!indexId) {
			...
			return endpoint.getOrPostCancelable<ParsePatternResponse>(
	...
	getTermAutocomplete: (indexId: string, annotatedFieldId: string, annotationId: string, prefix: string, requestParameters?: AxiosRequestConfig) => {
		return endpoint.getOrPostCancelable<string[]>(blacklabPaths.autocompleteAnnotation(
			indexId,
			annotatedFieldId,
			annotationId
		), {
