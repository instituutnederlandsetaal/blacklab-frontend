...
export const isLoadableLike = <T>(v: any): v is LoadableLike<T> =>
	v != null && typeof v === 'object' && 'state' in v && v.state in LoadableState && 'value' in v && 'error' in v;

export const isLoadableBase = <T>(v: any): v is LoadableBase<T> =>
	v != null && typeof v === 'object' && 'isLoading' in v && 'isLoaded' in v && 'isError' in v && 'isEmpty' in v;
...
export function thisIsLoading(this: any): boolean { return isLoading(this); }
...
export function thisIsLoaded(this: any): boolean { return isLoaded(this); }
...
export function thisIsError(this: any): boolean { return isError(this); }
...
export function thisIsEmpty(this: any): boolean { return isEmpty(this); }
...
export const wrap = <
	T,
	VT extends ValueTypeFromLoadableOrObservable<T> = ValueTypeFromLoadableOrObservable<T>
>(value: T): Loadable<VT> =>
...
export const Loading = <T = never>(): Loading<T> =>
	loadable<T>(LoadableState.loading, undefined, undefined) as Loading<T>;

export const Loaded = <T>(value: T): Loaded<T> =>
	loadable<T>(LoadableState.loaded, value, undefined) as Loaded<T>;

export const LoadingError = <T = never>(error: ApiError): LoadingError<T> =>
	loadable<T>(LoadableState.error, undefined, error) as LoadingError<T>;

export const Empty = <T = never>(): Empty<T> =>
	loadable<T>(LoadableState.empty, undefined, undefined) as Empty<T>;
...
export type Val<T> =
	T extends Loaded<infer LT> ? LT :
	T extends Loading<infer LT> ? LT :
	T extends Empty<any> ? never :
	T extends LoadingError<any> ? never :
	T extends Loadable<infer LT> ? LT :
	T;
...
export type ValEmpty<T> =
	...
	T extends Loadable<infer LT> ? LT | undefined :
	T;
...
export type ValError<T> =
	...
	T extends Loadable<infer LT> ? LT | ApiError :
	T;
...
export type ValEmptyAndError<T> =
	...
	T extends Empty<unknown> ? undefined :
	T extends LoadingError<unknown> ? ApiError :
	T extends Loadable<infer LT> ? LT | undefined | ApiError :
	T;


export type ValueTypeFromLoadableOrObservable<T> = Val<
	T extends ArrayLike<any> ? T : T extends ObservableInput<any> ? ObservedValueOf<T> : T
>;
