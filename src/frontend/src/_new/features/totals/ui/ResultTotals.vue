...
import { IterativeResultCountLoader } from '@/_new/features/totals/lib/result-count-from-query';
import type { TotalsOutput } from '@/_new/features/totals/lib/result-count-helpers';
import type { BLSearchResult } from '@/_new/types/blacklabtypes';

import type { ApiError } from '@/_new/shared/api/lib/api-types';
import { frac2Percent } from '@/_new/shared/utils/numbers-utils';
...
export default defineComponent({
	...
	props: {
		initialResults: {
			required: true,
			type: Object as PropType<BLSearchResult>,
		},
		type: {
			required: true,
	...
	computed: {
		...
		searchTime(): string {
			return this.value ? this.frac2Percent(this.value.searchTime / 100000, 1).replace('%', 's') : '';
		},
