...
const elementMarker = 'data-vue-meta';

function _useInsertableContent<T>(p: {
	...
	watch(() => toValue(p.content), (content) => {
	...
	return _useInsertableContent({
		...
		insert: (cssEntries) => {
			cssEntries.forEach(css => {
				...
				link.setAttribute(elementMarker, '');
				document?.head?.appendChild(link);
			});
		},
		remove: () => document?.head?.querySelectorAll?.(`link[${elementMarker}]`)?.forEach(e => e.remove())
	});
}
	...
	return _useInsertableContent({
		...
		insert: (jsEntries) => {
			jsEntries.forEach(js => {
				...
				script.setAttribute(elementMarker, '');
				document?.body?.appendChild(script);
			});
		},
		remove: () => document?.body?.querySelectorAll?.(`script[${elementMarker}]`)?.forEach(e => e.remove())
	});
}

export { useFavicon } from '@vueuse/core';
