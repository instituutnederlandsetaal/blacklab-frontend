 ...
 */
export function createLegacyCustomizationApi(corpus: AppTypes.Corpus) {
	const customizations = {
		search: {
			...
			metadata: {
				...
				_customTabs: dontProxy([] as LegacySearchFilterTab[]),

				/** Add a custom tab with some (span) filter fields */
				addCustomTab(name: string, fields: AppTypes.FilterDefinition<unknown>[]) {
					const validFields = fields.filter(field => field?.id);
					validFields.forEach(filter => FilterStore.actions.registerFilter({ groupId: name, ...filter }));
					this._customTabs.push({ name, fields: validFields });
				},
	...
	return wrapWithErrorHandling(customizations);
}

export type LegacyCustomizationApi = ReturnType<typeof createLegacyCustomizationApi>;
