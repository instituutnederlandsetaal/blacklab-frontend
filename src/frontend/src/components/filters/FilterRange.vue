<template>
	<div class="form-group filterfield" :id="htmlId" :data-filterfield-type="definition.componentName">
		...
		>
		<Debug v-else
			><label>(id: {{ id }})</label></Debug
		>
		<div class="col-xs-4">
			<input type="text" :placeholder="$t('filter.range.from')" class="form-control" autocomplete="off" :id="inputId + '_lower'" v-model="lower" />
		</div>
		<div class="col-xs-4">
			<input type="text" :placeholder="$t('filter.range.to')" class="form-control" autocomplete="off" :id="inputId + '_upper'" v-model="upper" />
		</div>
