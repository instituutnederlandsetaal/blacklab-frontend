...
export function firstNonLoaded<T extends MaybeLoadablesArrayOrObject>(loadables: T): PassthroughFrom<T> | undefined {
...
export function extractLoadedValues<T extends MaybeLoadablesArrayOrObject>(loadables: T): LoadedValues<T> {
...
export function mapLoadedValue<T extends MaybeLoadablesArrayOrObject, U>(loadables: T, mapper: (values: LoadedValues<T>) => U): PassthroughFrom<T> | Loadable<U>;
export function mapLoadedValue<T extends MaybeLoadablesArrayOrObject, U>(loadables: T, mapper: (values: LoadedValues<T>) => U): PassthroughFrom<T> | Loadable<U> {
...
export function flatMapLoadedValue<T extends MaybeLoadablesArrayOrObject, U>(loadables: T, mapper: (values: LoadedValues<T>) => MaybeLoadable<U>): PassthroughFrom<T> | MaybeLoadable<U>;
export function flatMapLoadedValue<T extends MaybeLoadablesArrayOrObject, U>(loadables: T, mapper: (values: LoadedValues<T>) => MaybeLoadable<U>): PassthroughFrom<T> | MaybeLoadable<U> {
	const combined = combineLoadablesValue(loadables);
	if (!isLoaded(combined)) return combined as PassthroughFrom<T>;
	const loadedValue = combined.value as LoadedValues<T>;
