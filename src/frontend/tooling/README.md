...
- `*.config.ts` and `knip.json` are the shared tooling configuration.
- `lint.js` runs the type, lint, Knip, and abstraction checks.
- `url-sync-smoke.js` is the Docker/Playwright smoke test.
